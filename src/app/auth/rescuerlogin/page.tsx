...
export default function RescuerRegister() {
  ...
  async function onSignUpSubmit(data: z.infer<typeof signUpSchema>) {
    console.log("onSignUpSubmit called with data:", data);
    startTransition(async () => {
      const { signUpRescuer, signInRescuerWithEmailAndPassword } =
        useRescuers();
      ...
      // const result = await signUpRescuer(data);
      const result = await signInRescuerWithEmailAndPassword(data);
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl className="w-full text-sm px-4 h-[45px] rounded-xl bg-lightBorder text-black border border-lightBorder">
